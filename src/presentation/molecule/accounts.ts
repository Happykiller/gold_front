...
export function formatAccounts(accounts: AccountUsecaseModel[]): FormattedAccount[] {
  const map = new Map<number, FormattedAccount>();

  // Clone accounts and initialize children and aggregates
  accounts.forEach(acc => {
    map.set(acc.id, { ...acc, children: [], balance_reconcilied_aggregate: acc.balance_reconcilied, balance_not_reconcilied_aggregate: acc.balance_not_reconcilied });
  });
